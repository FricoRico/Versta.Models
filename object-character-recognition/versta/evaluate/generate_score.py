...
def generate_result_tables(
    ...
    scores["text_block_Edit_dist"] = data["text_block"]["all"]["Edit_dist"].get("ALL_page_avg", float("nan"))
    scores["reading_order_Edit_dist"] = data["reading_order"]["all"]["Edit_dist"].get("ALL_page_avg", float("nan"))

    # CDM metric uses page ALL (0-1, higher is better)
    cdm_value = data["display_formula"]["page"].get("CDM", {}).get("ALL")
    scores["display_formula_CDM"] = cdm_value * 100 if cdm_value is not None else float("nan")
    ...
    teds_structure_value = data["table"]["page"].get("TEDS_structure_only", {}).get("ALL")
    scores["table_TEDS_structure_only"] = teds_structure_value * 100 if teds_structure_value is not None else float("nan")

    # Overall score: ((1 - text_block_Edit_dist) * 100 + display_formula_CDM + table_TEDS) / 3
    # All on 0-100 scale, higher is better
    if not any(v is None or (isinstance(v, float) and v != v) for v in [
