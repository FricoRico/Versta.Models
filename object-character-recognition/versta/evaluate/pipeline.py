...
def _crop_text_regions(
    image: np.ndarray, polygons: List[np.ndarray]
...
def run_pipeline(
    ...
    if is_quantized:
        ...
        recognizer = ONNXRecognizer(
            recognizer_model_path, model_dir, num_threads=actual_threads
        )
    else:
    ...
    overall_confidence = (detect_confidence or 0.5 + recognize_confidence or 0.5) / 2

    return recognize_result, overall_confidence


def perform_detection(
    image_path: str, detector, batch_size: int, is_quantized: bool
            ...
            for result in results:
                ...
                for text, conf in zip(rec_texts, rec_scores):
                    all_results_serializable.append(
                        {"rec_text": str(text), "rec_score": float(conf)}
                    )
        ...
        for box in boxes:
            ...
            try:
                ...
                for result in results:
                    all_results.append(result)
                    conf = result.get(
                        "rec_score", result.get("res", {}).get("rec_score", 0.0)
                    )
    ...
    for r in all_results:
        ...
        if isinstance(rec_text, list):
            for text, score in zip(rec_text, rec_score):
                all_results_serializable.append(
                    {"rec_text": str(text), "rec_score": float(score)}
                )
                all_confidences_serializable.append(float(score))
        else:
            all_results_serializable.append(
                {"rec_text": str(rec_text), "rec_score": float(rec_score)}
            )
            all_confidences_serializable.append(float(rec_score))

    avg_confidence = (
        sum(all_confidences_serializable) / len(all_confidences_serializable)
        if all_confidences_serializable
        else 0.0
    )
