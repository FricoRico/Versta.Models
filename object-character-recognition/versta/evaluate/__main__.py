...
def parse_args():
    parser = ArgumentParser(description="OmniDocBench-aligned OCR evaluation")
    ...
    parser.add_argument(
        "--subset", type=int, help="Number of samples to evaluate (default: all)"
    )
...
def run_inference_and_eval(args):
    ...
    model_types = (
        [args.model_type] if args.model_type != "both" else ["paddleocr", "onnx"]
    )
