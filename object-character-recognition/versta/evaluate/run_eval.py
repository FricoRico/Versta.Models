        ...
        config = {
            "end2end_eval": {
                ...
                "dataset": {
                    ...
                    "ground_truth": {
                        "data_path": str(gt_json_path)
                    },
                    "prediction": {
                        "data_path": str(predictions_dir)
                    },
    ...
    if metric_files:
        with open(metric_files[0], 'r') as f:
            ...
            if "display_formula" in data:
                metrics["formula_edit_dist"] = data["display_formula"].get("Edit_dist", {})
            if "table" in data:
                metrics["table_edit_dist"] = data["table"].get("Edit_dist", {})
            if "reading_order" in data:
                metrics["reading_order_edit_dist"] = data["reading_order"].get("Edit_dist", {})
